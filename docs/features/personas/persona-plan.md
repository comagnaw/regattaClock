...
2. **Persisting the result — one commit per team.** The approval panel serializes the lap rows into a `RaceResult` and atomically rewrites `finish.json` ([README.md](README.md) FT privileges); each FT persona has exactly one control that does it.
   ...
   A **status line** under the panel reads `Pending` until the race is persisted, then `Approved at <date> by <host>` (primary) or `Saved at <date> by <host>` (secondary) — the local timestamp in RFC 1123 form from `RaceResult.ApprovedAt` / `UpdatedAt`, and `FinishLog.Envelope.Machine`.
