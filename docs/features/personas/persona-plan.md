...
- **Event logging is foundational.** Wire `PrefLogging` / `PrefDebug` early via `internal/applog` so every later phase can emit structured JSON logs as it lands, rather than retrofitting call sites at the end. Full design in section 6c; deeper rationale in [logging-options.md](closed/logging-options.md).
...
- **Primary/Secondary Finish Timer** writes only `timing/<team>/finish.json`, which *is* the race results file. Reads `director/regattaSchedule.json` and `timing/<team>/start.json`. The **primary** FT additionally reads `timing/secondary/finish.json` read-only, to feed its Compare Secondary window (section 9); it never writes it.

The Director never writes results. Results are produced by the Finish Timer: the primary FT writes on **Referee Approval** (`Approved: true`), the secondary FT writes on **Save and Close** (`Approved: false`) ([README.md](README.md) Finish Timer privileges). The RD is a consumer of results, not a producer: it reads the **primary** team's `start.json` + `finish.json` for its progress tree and exports lane images. Combining the two teams' `finish.json` into one published set is left to a future results/publish persona ([reconciliation.md](closed/reconciliation.md)); the primary FT reconciles the secondary's numbers into the primary `finish.json` by hand, aided by the Compare Secondary window.
...
2. **On fingerprint change: load and normalize, do not write yet.** Re-read the origin through `internal/reader` into a candidate schedule (slim fields only — see [schedule-data-model.md](closed/schedule-data-model.md)).
...
- **Later:** an **in-app-authored heat sheet** (`Origin.Type = "heatsheet"`), built by the RD / Heat Sheet Author from the RegattaCentral roster, that produces a `store.Schedule` directly — fingerprinted by `Schedule.ContentHash()`, dispatched alongside `"excel"` in `setRegattaData` / `pollOrigin`. RegattaCentral is a **roster data source only** — it has no heat sheet — so the API is not itself a schedule origin; see [regattacentral-integration.md](regattacentral-integration.md#phase-c--in-app-heat-sheet-authoring). Same RD UX either way: origin fingerprint change → load → **semantic compare** → prompt/Apply only if schedule content differs → `regattaSchedule.json`.
- **`regattaSchedule.json` remains the shared contract** for all personas. Timers never talk to Excel, the authoring UI, or RegattaCentral; only the RD’s origin adapter does. That is what keeps an origin pivot from rewriting ST/FT.
...
4. **Persisted `finish.json`:** leave results as-is. Every write **does stamp** a per-race lane-map fingerprint on the `RaceResult` (`RaceResult.LaneMapHash`, `store.ScheduleRace.LaneMapHash()` — `RaceNumber` plus each lane’s school and additional info in lane order; class and flight excluded, since they change the title, not which boat is in which lane). This is written as of phase 7 so the data exists; **consuming it** — comparing the stored hash against the live schedule to surface a persistent “approved against an older lane map” mark that survives an app restart, in the FT race tree and the RD progress tree — is **built** as of phase 8d (§10; `staleLaneMap` / `finishResultStale` in [stale_lanes.go](../../internal/regatta/stale_lanes.go)). The in-session diff (§3c items 1–3) still applies for schedule changes that arrive while the FT's clock is open.
...
Do **not** add a field named `Persona` on this type (package is already `persona`; use `ID`). Logs use JSON key `persona_id` for the same value — see [logging-options.md](closed/logging-options.md).
...
- **Finish Timer** reads both `timing/<team>/start.json`, for the start-time value on each row, and its own `timing/<team>/finish.json`, for the races it has already timed. Combined with the per-race clock rehydration in section 9, reopening any race returns the FT to exactly the state they left it. The **primary** FT also hydrates `timing/secondary/finish.json` read-only for the Compare Secondary window.
- **Director** reads `director/regattaSchedule.json` plus the **primary team's two** timing files, as described in section 9.
...
- **Start Timer**: title, start-time label, `Start Time` button, `Clear` button, and a `Restore` button. No `Time Race` button. `Clear` still prompts for confirmation ([README.md](README.md)), and recording, clearing, and restoring each write `start.json` immediately.
  ...
  **The ST row locks once the FT begins timing that race.** As soon as a `RaceResult` exists for the race in the team's `finish.json` — written from the FT's clock **Start** click onward (see the Finish Timer clock notes below) — the ST's `Start Time` / `Clear` / `Restore` buttons for that race are disabled and the mutators no-op. The rationale is section 2.1: after the FT's Start click the winning time is being computed as `FT Start − ST start time`, so a later change to the start time silently corrupts a result in progress. The ST observes this by **mirroring `finish.json` read-only** (a read, like the FT reading `start.json`); it never writes `finish.json`. The lock releases if the FT abandons the race before saving. The Status cell carries the shared race-progress word — "timing in progress", then "saved", then "approved" — the same vocabulary the FT and Director trees use; the disabled buttons, not the wording, carry the "locked" meaning. There is deliberately no explicit ST lock control — a lock the ST must remember to set has the same human-error failure mode it exists to prevent.
...
- **Director**: read-only. No buttons on any row. Each row carries the race title plus four values that together show how far the regatta has progressed: **Restarts**, **Start Time**, **Winning Time**, and an **approval indicator**.
  ...
  **Primary team only.** The tree renders the primary team's values; a race the primary pair has not started shows placeholders until the primary FT reconciles the secondary's numbers into the primary `finish.json` (aided by the Compare Secondary window; see [reconciliation.md](closed/reconciliation.md)). An earlier draft did per-value primary→secondary fallback with a `·2nd` marker and legend (phase 8b-2); that was removed — an RD who reads a secondary time as though it were primary has worse information than a blank cell, and moving the secondary comparison to the PFT clock keeps it in front of the person doing the reconciliation. This tree is **oversight**, not the published result set; how the two teams' `finish.json` files are combined into what gets published is specified in [reconciliation.md](closed/reconciliation.md).

  This makes the Director the one person who can see the regatta stalling, so it is where the staleness indicator and the clock-skew banner from section 2.1 matter most — now scoped to the primary team's `start.json` vs `finish.json` (primary ST machine vs primary FT machine). Skew between the primary and secondary FT machines surfaces in the PFT Compare Secondary window instead.

  Two consequences worth noting. The RD no longer times races at all — dropping the row buttons removes the `Time Race` path from the Director, leaving Excel import and lane-image export in the menus as its only actions. And **the Director now needs the watcher**, which the earlier draft scoped to timers only; it watches the primary team's two timing files.
...
Changes to [internal/clock](internal/clock) — the first three below, plus a
read-only **Compare Secondary** window for the primary FT (this section), the
persona role in the window title (`Race N Clock — Primary Finish Timer`), and a
banded-zone visual pass (a "Timing" accent band carrying the race title + the
regattaClock wordmark as a masthead, over a "Results" band and a
reverse-contrast lanes card):
...
4. **Compare Secondary (primary FT only).** A toggle in the approval panel opens an independent, non-blocking window that renders the **secondary** team's committed `RaceResult` for the same race read-only, laid out to parallel the clock (masthead, the secondary's last split where the clock shows its stopwatch, a read-only lap grid, the winning-time line, the lanes table), in **amber** bands so it cannot be mistaken for the live clock. An amber caution appears when the two machines' measured offsets differ by more than `timesync.SkewWarnThreshold` (section 2.1), since the raw winning times are not directly comparable across machines. The primary FT hydrates `timing/secondary/finish.json` read-only at startup and watches it on the same watcher that keeps the ST start times live; a fresh secondary save enables the toggle and refreshes an open window with no user action. This is purely a visual aid — the primary FT reconciles by editing its **own** `finish.json` (the digital form of the old "read the secondary's numbers off and hand-type them" step); the secondary file is never written here. Closing the window returns focus to the clock.
...
3. **Layout and store types** — `internal/persona/store` types; slim `regattaSchedule.json` per [schedule-data-model.md](closed/schedule-data-model.md); migration from `data.json`; log hydrate/write success and ERROR on failure.
...
8. **Regatta Director** — sliced:
   ...
   - **8b. Read-only progress tree** — *(done)* **8b-1**: shared column grammar + a per-column header row for all three trees; RD tree on the `raceRow` map with the four metric columns. **8b-2**: the Director hydrates the **primary team's** `start.json` + `finish.json`, watches three files (schedule + primary start + primary finish), and renders Restarts / Start Time / Winning Time / approval from the primary team; dismissible clock-skew banner (primary ST machine vs primary FT machine, section 2.1) and staleness banner (freshest write older than 10 min, section 9). Executive-team log path already lands with 8a. *(As first shipped, 8b-2 did per-value primary→secondary fallback with a `·2nd` suffix + legend across all five files; that was later removed — the RD tree is primary-only and the secondary comparison moved to the PFT clock, section 9 and reconciliation.md.)*
   ...
   - **8d. Consume `RaceResult.LaneMapHash`** *(done)* (section 3c item 4): `staleLaneMap(n)` compares a committed result's stored hash (winning time or approval present; a pre-8d result with no stamp is never flagged) against `raceLaneMapHash(live race n)`. A mismatch prefixes the row title with `common.StaleLaneMapMark` ("† ") in the FT tree (own `finish.json`) and the RD tree (the primary team's result), with an italic header legend shown only while a row is flagged. Survives a restart because the hashes are on disk; clears when the race is reopened and re-Saved.
   - **8e. Reconciliation** *(settled)* — the primary FT's read-only **Compare Secondary** window (section 9) is the accepted solution: see the secondary's committed result, re-key anything that needs reconciling into the primary `finish.json` by hand. No further SFT→PFT reconciliation work is planned. The [reconciliation.md](closed/reconciliation.md) read-time auto-selection model (per-race verdict column, disputed-race resolution screen, provenance-tagged publish) stays on file as the spec **if** a future results/publish persona is ever built; it is not scheduled.
...
- **Director progress tree** — restarts reflect `len(Cleared)`; values come from the primary team only, and a race the primary pair has not started shows placeholders (no secondary fallback).
...
- **Add a Windows CI job** *(done)*: [.github/workflows/test.yml](.github/workflows/test.yml)'s `test-windows` (OS-portable packages, every PR) and `test-windows-full` (Fyne/CGO packages, post-merge) jobs run on `windows-latest`.
- **Sanitize filenames** *(done)*: `internal/filesystem/sanitize.go`'s `SanitizeForFilename` is wired into both places that build a filename from free-text data — the regatta name in [exporter.go](../../internal/exporter/exporter.go) and the hostname in the per-persona log filename ([regatta.go](../../internal/regatta/regatta.go)).
...
- **Result-driven content personas.** Downstream of the results/publish persona: read-only operators that turn one approved race into published content (a social-media text table, the same as a PNG). [future-result-driven-persona.md](future-result-driven-persona.md) assesses whether Referee Approval should materialize a per-race file — recommendation is to defer and keep `finish.json` the only source of truth, reserving `regattaData/results/` (the name freed by the stage-3 cleanup) for a future single-writer materialized view carrying a per-race `revision` for re-publish detection. [sidecar-personas.md](sidecar-personas.md) refines this into a **sidecar capability** attached to a lead persona (Director / Timer) rather than a standalone persona — with a Lead / Standalone / Sidecar classification, a render-for-a-human first version, and a phased build plan.
- **Non-Excel schedule origin.** A generalised `ScheduleOrigin` (`Fingerprint()` / `Load()`) backing an **in-app-authored heat sheet** (`Origin.Type = "heatsheet"`) that the RD / Heat Sheet Author builds from the RegattaCentral roster — RegattaCentral has no heat sheet of its own, so it is a roster source feeding the authoring UI, not a schedule origin. Same detect → semantic-compare → Apply UX as Excel. Not part of the first persona ship; keep `SourceInfo` / fingerprint fields ready. See [regattacentral-integration.md](regattacentral-integration.md).

## 14. Closed items

- **Reconciliation.** *Settled* — the primary FT reconciles the secondary's numbers into the primary `finish.json` by hand, using the read-only **Compare Secondary** window (section 9). No further SFT→PFT reconciliation work is planned. The [reconciliation.md](closed/reconciliation.md) read-time auto-selection model stays on file as the spec for a future results/publish persona, not scheduled. The RD's progress tree shows the primary team only, with its skew / staleness banners and stale-lane-map mark scoped to that team.
- **Cross-team fallback.** *Settled* — not pursued. If the primary ST never records a start time, the primary FT is not blocked: the derived winning time (section 2.1) is only a pre-fill convenience, and the **Winning Time** field always accepts the referee's on-water time as a manual override regardless of whether any `start.json` value ever arrives. An automatic fallback to the secondary team's `start.json` would silently cross the primary/secondary boundary for a corner case the existing manual-entry path already covers.
- **ST locked on the wrong race.** *Settled* — no RD override planned. The FT already owns releasing the lock it created (reopen/clear the race, section 9), and because the primary FT is the final approver of every result — always free to enter **Winning Time** manually from the on-water referee rather than depend on the derived ST value — a race locked against the wrong start time still does not block a correct result from being recorded. Not worth an RD-initiated override for this corner case.
- **RD opens an unrelated workbook.** *Resolved* — a `RegattaKey` mismatch check now guards every schedule write (`classifyScheduleWrite` / `guardScheduleWrite` / `setAsideSchedule` in [schedule_guard.go](../../internal/regatta/schedule_guard.go)), matching phase 8c-1 (section 10): a matching key proceeds as a normal edit; a different key with no timing data on disk confirms and keeps the outgoing file as `regattaSchedule.<oldKey>.json`; a different key *with* timing data is refused outright; an unreadable on-disk schedule is refused too. This is the RD-side mirror of the timer-side "reject data from a different regatta" rule (section 8).
- **Challenge codes in source.** *Resolved* — `internal/personacfg`'s `Config.Challenges` plus `matchesChallenge` in [persona_config.go](../../internal/regatta/persona_config.go) let a deployment config file (`PrefPersonaConfigFile`) replace any persona's built-in `rc-*` code with no release needed. See [persona-config-file.md](persona-config-file.md). The mechanism differs from this item's original sketch — a general deployment config file the operator points at, rather than something the RD writes into `regattaData/director/` — but it satisfies the same need.
- **Local write-ahead journal.** *Built* — `internal/persona/journal`: `store.SaveStart`/`SaveFinish` durably stage every write to a local cache directory before flushing it to the shared path in the background, with capped exponential-backoff retry and crash recovery on restart (a write staged but never flushed before an unclean shutdown is picked back up at the next `hydrateOwnStart`/`hydrateOwnFinish`). A race-tree banner (`journalStatusBanner`) surfaces a stuck flush with a "Retry Now" action. Scoped to `SaveStart`/`SaveFinish` only, per [shared-storage-options.md](shared-storage-options.md)'s recommendation; the Director's occasional `SaveSchedule` import still writes directly.
- **Timer-side staleness indicator.** *Settled* — not pursued. A "start times last updated Ns ago" banner on the Timer's own race tree would only ever flag a stale `start.json`, and the primary FT is the final approver of every result: **Winning Time** always accepts the referee's on-water time as a manual override regardless of whether or when a start time arrives (section 2.1). With that safety net already in place, a Timer-side freshness banner has no action for the operator to take on it — it would just be noise. The RD-side staleness banner (section 9, `checkDirectorStale`) is unaffected: it is fleet-wide "is anyone still sending data" monitoring, a different concern from a single FT deciding whether to trust its own derived winning time.
- **Director-initiated `w32tm /resync` button.** *Settled* — not pursued. Section 2.1's offset-measurement approach (`internal/timesync`, `github.com/beevik/ntp`) already handles clock skew without touching the system clock; this was only ever a secondary escape hatch, and it was specifically sketched as a button on the Director's config screen. The Director has no timing functionality — it never records a start or finish time — so clock skew between timing-team machines is not something the Director experiences or needs to correct. No persona is left to host this button.
- **Log rotation and remote syslog export.** *Settled* — not pursued. Both were floated in [logging-options.md](closed/logging-options.md) (now closed) as later niceties for bounding log file growth and centralizing logs off-machine. Neither is needed: INFO-level events fire only on discrete actions (clicks, NTP measures, watcher content changes), so an all-day regatta's per-persona log stays in the low single-digit MB, and `internal/applog`'s `O_APPEND` writer already appends in constant time regardless of file size — large or small, it never blocks the timing path or the RD's watcher (which never touches `logs/` in the first place). No rotation or remote export is planned unless an actual need shows up. Log collection/visibility itself is unaffected by this decision — it is scoped to the **Developer (DEV)** persona proposal (`docs/features/TODO.md`, [developer.md](new/developer.md)), not the Director, and is a support convenience rather than a response to a size or performance problem.
