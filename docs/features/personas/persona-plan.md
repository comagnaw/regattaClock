...
7. **Clock integration** — derived winning time, save on approve/save (plus an in-progress `RaceResult` on the clock's Start click), rehydration, skew warnings; log clock actions and ERROR on save failure; schedule-conflict label refresh while clock open (section 3c). **Start Timer lock** — the ST mirrors `finish.json` read-only and disables `Start Time` / `Clear` / `Restore` for any race the FT has begun timing (section 9).
