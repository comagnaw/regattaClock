...
8. **Regatta Director** — sliced:
   ...
   - **8e. Reconciliation model** — [reconciliation.md](reconciliation.md): the two teams' `finish.json` files are combined by **read-time auto-selection**, primary then secondary per race, each published race carrying a provenance tag (`primary` / `secondary` / `disputed`), with gaps the primary missed closed by the primary FT re-entering the secondary's numbers. The RD persists no reconciliation decision. *Shipped:* the primary FT's read-only **Compare Secondary** window (section 9) — the surface that lets the primary FT see the secondary's committed result and re-key it. *Still design-only:* the read-time auto-selection consumer — a per-race verdict column, a disputed-race resolution screen, and the export/publish with places and times — which lands with a future results/publish persona.
...
- **Reconciliation.** When both teams time the same regatta, primary and secondary results are combined into the published set by **read-time auto-selection with a provenance tag** — model in [reconciliation.md](reconciliation.md) (phase 8e). The consumer (a per-race verdict column, a disputed-race resolution screen, and the export/publish itself) is future work with a results/publish persona. The shipped surface is the primary FT's read-only **Compare Secondary** window (section 9); the RD's progress tree shows the primary team only, with its skew / staleness banners and stale-lane-map mark scoped to that team.
