...
8. **Regatta Director** — sliced:
   ...
   - **8c. Schedule origin refresh** (section 3b). **8c-1** *(done)*: the **RegattaKey-mismatch guard** on the RD import/reload path — same key writes; a different key with no timing data confirms and keeps the old file as `regattaSchedule.<oldKey>.json`; a different key *with* timing data is refused (the RD archives/clears the folder or points at a fresh one); an unreadable on-disk schedule is refused too. Plus a **Reload Schedule** menu item that re-reads the recorded workbook through the same confirm + guard. **8c-2**: background origin-fingerprint poll → re-read → canonical semantic schedule compare → persistent "origin has meaningful changes" banner with Apply / Dismiss; a no-op workbook save must not rewrite `regattaSchedule.json`.
