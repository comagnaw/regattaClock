...
- **Result-driven content personas.** Downstream of the results/publish persona: read-only operators that turn one approved race into published content (a social-media text table, the same as a PNG). [future-result-driven-persona.md](future-result-driven-persona.md) assesses whether Referee Approval should materialize a per-race file — recommendation is to defer and keep `finish.json` the only source of truth, reserving `regattaData/results/` (the name freed by the stage-3 cleanup) for a future single-writer materialized view carrying a per-race `revision` for re-publish detection.
