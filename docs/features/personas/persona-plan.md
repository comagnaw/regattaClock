...
The Director never writes results. Results are produced by the Finish Timer: the primary FT writes on **Referee Approval** (`Approved: true`), the secondary FT writes on **Save and Close** (`Approved: false`) ([README.md](README.md) Finish Timer privileges). The RD is a consumer of results, not a producer: it reads the **primary** team's `start.json` + `finish.json` for its progress tree and exports lane images. Combining the two teams' `finish.json` into one published set is left to a future results/publish persona ([reconciliation.md](reconciliation.md)); the primary FT reconciles the secondary's numbers into the primary `finish.json` by hand, aided by the Compare Secondary window.
...
8. **Regatta Director** — sliced:
   ...
   - **8e. Reconciliation** *(settled)* — the primary FT's read-only **Compare Secondary** window (section 9) is the accepted solution: see the secondary's committed result, re-key anything that needs reconciling into the primary `finish.json` by hand. No further SFT→PFT reconciliation work is planned. The [reconciliation.md](reconciliation.md) read-time auto-selection model (per-race verdict column, disputed-race resolution screen, provenance-tagged publish) stays on file as the spec **if** a future results/publish persona is ever built; it is not scheduled.
...
- **Reconciliation.** *Settled* — the primary FT reconciles the secondary's numbers into the primary `finish.json` by hand, using the read-only **Compare Secondary** window (section 9). No further SFT→PFT reconciliation work is planned. The [reconciliation.md](reconciliation.md) read-time auto-selection model stays on file as the spec for a future results/publish persona, not scheduled. The RD's progress tree shows the primary team only, with its skew / staleness banners and stale-lane-map mark scoped to that team.
