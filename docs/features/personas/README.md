...
- **Primary FT — Publish (results publishing):** a per-race **Publish** button beside **Time Race**. It is enabled once the race is approved (`store.CanPublish`) and regenerates the RegattaClock-owned results workbook, `<Name> - <Date> Results.xlsx`, in the manual **Results** worksheet layout. The button reads **Publish**, then **Published** while the workbook matches the approved result, then a highlighted **Re-publish** after the result changes.
  - **Folder:** the results folder is confirmed per regatta at session start (declinable) or on the first Publish, and is recorded in the PFT's own `finish.json` (`ResultsDir`). A new regatta never inherits the previous one's folder. This machine's last-used folder is only offered as the default.
  - **No results copy:** there is no second copy of results data. The workbook is rendered from `finish.json` + the schedule, and a hidden ledger sheet in the workbook records what was published and for which regatta.
  - **Refusals:** it will not overwrite a workbook it didn't write or one from another regatta.
  - **RegattaCentral** is a planned second destination, deferred — see [new/results-publisher.md](new/results-publisher.md).
