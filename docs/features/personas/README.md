...
- **Primary FT:** no standalone Save — **Referee Approval** is the only commit (`Approved: true`) and it leaves the clock open so a correction can be re-approved. The **Close** button is disabled until the race is approved. A status line shows `Pending` → `Approved HH:MM:SS`.
- **Secondary FT:** no Referee Approval step — **Save and Close** is the terminal action: it writes results unapproved (`Approved: false`) and closes the clock. Its status line shows `Pending` → `Saved HH:MM:SS`. The secondary `finish.json` is a backup data source for the primary FT and reconciliation ([reconciliation.md](reconciliation.md)); the primary FT is the only path to an approved result.
