...
## Testing — sample regattaData (proposed, after REP)

- [ ] Developer flag `-dev-sample-regatta <dir>` generating a full-day, obfuscated regattaData (last five races un-raced) from a one-time `.xlsm` ingest, for full-size multi-machine testing and an end-of-day finish run. Sequenced after Results Publisher's local `.xlsx` results write lands, reusing its Results-layout definition and writer — [sample-regatta.md](testing/sample-regatta.md)
