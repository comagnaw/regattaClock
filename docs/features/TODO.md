...
- [ ] Race tree: show each race's lane/boat count as its own column, the same way the scheduled start time got its own "Scheduled Time" column rather than being concatenated into `RaceTitle()` — `reader.RaceData.BoatCount` already exists, this is purely a display addition following that same pattern — [regattaData.go](../../internal/reader/regattaData.go)
...
- [ ] **Race state machine & attribute ownership** (architecture, not a persona) — the canonical race-lifecycle state machine (pre-timing → per-team timing → official approval → downstream consumption, plus HSC v3's cross-race progression edge), the full attribute-ownership table, an explicit in-memory-vs-watcher rule, and a "pane of glass" race-tree redesign (one canonical `store.TeamState`/`CanPublish` pair replacing today's duplicated `raceProgressStatus`/`clock.commitState`, plus a shared `publish.IsStale` staleness helper). Supersedes `reconciliation.md`'s state-transition ladder (its verdict/disputed model is retired, not reused — see that doc's own updated Status). No architectural blocker beyond the feature freeze; **the next feature to land once it lifts, before any of the persona implementations below begin** — every one of them should build against the helpers this doc defines from the start — [race-state-machine.md](personas/race-state-machine.md)
- [ ] **Heat Sheet Creator (HSC)** — Executive-team, **standalone persona**, not a sidecar. Foundational: originates regatta data from RegattaCentral (roster/events/entries) so the RD can turn it into `regattaSchedule.json`, replacing the current manual RC-export-to-Excel process. Staged: **v1** (read-only local working copy, participant/waiver filtering, RC-id fields added directly to `store.ScheduleRace`/`store.ScheduleEntry`, deadline/finalize workflow, fully-manual post-finalize sync) has no blockers and does not depend on the RC write-path investigation; **v2** (pushing the finalized Heat Sheet back to RC) is hard-gated on that investigation concluding; **v3** (heat→semi/final progression seeding, per a VASRA reference algorithm — computes a later race's lane draw from an earlier heat's approved result and proposes it for the RD to apply) is a separate later increment with no RC dependency, but needs its own design pass for round-type/progression metadata and placeholder-race creation first. The editing-UI question (Fyne in-app authoring, resolved by a dedicated throwaway feasibility spike, vs. generate-and-reconcile-Excel) is deliberately left open pending that spike. Does not block Awards/Developer/Results Publisher/SOM/Streamer, but its RC-id indexing decision should inform their own later RC-dependent slices — [heat-sheet-creator.md](personas/new/heat-sheet-creator.md)
...
- [ ] **Awards (AWD)** — Executive-team, standalone, read-only. Emulates the RD's primary-team progress tree with a per-race "View Results" button (enabled once `RaceResult.Approved`) opening a read-only results window, modeled on Compare Secondary, for transcribing 1st/2nd/3rd place. No blockers found; shares the `Session.WritePath()` gap below with the deferred Result-publishing personas — [awards.md](personas/new/awards.md)
