...
- [ ] **Regatta operational state** (not a persona — shared config) — the
  ...
  platform selection for the still-deferred Social Post work. A dependency
  of Results Publisher below and of any future social-publishing persona,
...
- [ ] **Result-publishing personas** (social-text / social-image, aka Social Post / SOM) — Executive-team, read-only, each with its own challenge. Decision so far: **defer**. Needs a new `persona.Role` with `File: ""`, a `Session.WritePath()` branch, the `regattaData/results/` schema + a re-publish `revision` contract, where published state is recorded, and (now) the `SocialPlatforms` selection from Regatta operational state above — [future-result-driven-persona.md](personas/future-result-driven-persona.md#what-to-reserve-now-no-code), [sidecar-personas.md](personas/sidecar-personas.md#recommended-increments)
