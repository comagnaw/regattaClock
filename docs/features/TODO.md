...
- [ ] **Register Results** — automated push to RegattaCentral (sidecar increment 1). Needs: non-secret config + secret storage (OS keyring), an outbox / retry design, a field-mapping spec (`lane:` then `result:` records, race `status` lifecycle); this is the app's first outbound HTTP and first stored secret — [regattacentral-integration.md](personas/regattacentral-integration.md#write-integration--register-results-phase-d), [sidecar-personas.md](personas/sidecar-personas.md#recommended-increments)
