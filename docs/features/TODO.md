...
- [ ] **Scratches erase the school's identity instead of recording it.** A scratched lane is represented today by an empty `ScheduleEntry.SchoolName` — indistinguishable from a lane that was never assigned a boat at all — because `internal/regatta/schedule.go`'s change-detector defines a scratch as exactly `SchoolName` going from present to blank, and the Excel importer has no scratch-specific parsing. Fix: an explicit scratch marker on `ScheduleEntry` (e.g. `Scratched bool`, or a richer `Status` mirroring `regattacentral.LaneStatus`'s `SCR`/`DNS`/`DNF`/`DSQ`/`RMV`/`EXC`/`NJ` vocabulary) that keeps `SchoolName` intact, plus updating the scratch-detector to key off it, plus deciding how a scratch is recognized coming out of the source Excel workbook in the first place — [schedule-data-model.md](personas/schedule-data-model.md#keep-schedule-sot)
...
- [ ] Remove dead FT-result fields from `internal/reader`: `RaceEntry.Place`/`Split`/`Time`, `RaceData.Saved`/`Approved`, and `RegattaData.ApproveRace()` (plus its one call site, `internal/clock/buttons.go`'s referee-approval flow). The persisted schedule (`store.ScheduleRace`/`ScheduleEntry`) never had these fields; the reader-layer types still declare them and `ApproveRace` still writes to `RaceData.Approved`, but nothing reads it back — the canonical approval state has been `store.RaceResult.Approved` (via `store.DeriveTeamState`/`store.CanPublish`) since race-state-machine.md landed — [schedule-data-model.md](personas/schedule-data-model.md#remove-from-schedule-move--already-in-finish)
