...
- **Persona event logging.** Analysis in [logging-options.md](logging-options.md): gate on existing `PrefLogging`; OS-agnostic `slog` **JSON lines** under `regattaData/logs/<team>/<role>-<hostname>.log`; fixed attrs `persona` / `team` / `role` / `machine` on every line (hostname also in the filename so two hosts cannot share a log); async writes; log clicks / NTP / watcher changes only. Not required to block the first persona ship, but cheap to add beside watcher and timesync.
