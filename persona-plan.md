...
- **Persona event logging.** Analysis in [logging-options.md](logging-options.md): gate on existing `PrefLogging`, OS-agnostic `slog` text files under `regattaData/logs/<team>/<role>.log`, async writes, log clicks / NTP / watcher changes only. Not required to block the first persona ship, but cheap to add beside watcher and timesync.
